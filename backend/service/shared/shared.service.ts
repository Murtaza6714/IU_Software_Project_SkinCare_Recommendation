...
class SharedServiceClass extends Services {
  ...
  async fetchAdminRecommendationById(query: any) {
    try {
      logger.info('FetchRecommendation admin started for %s');
      const cacheKey = `${collectionNames.PRODUCT_RECOMMENDATION}:${redisListType.GET_ONE}:fetchAdminRecommendationById:${JSON.stringify(query)}`
      const redisData = await getRedisValue(cacheKey)
      if(redisData) {
      logger.info('FetchRecommendation admin by filter completed for %s');
      return this.success({ message: "Recommendation ran successfully!!", statusCode: HttpStatus.ok, data: {productRecommendation: redisData.productRecommendation, user: redisData.userData } })
      }
      let arangoQuery = `
      LET userData = (
      ...
      logger.error(`Error in FetchRecommendation admin by filter ${JSON.stringify(error)}`);
      throw error
    }
  }
